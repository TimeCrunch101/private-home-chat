...
export class HomeRSA {
    ...
    async checkIfKeypairExists() {
        try {
            await fs.access("./server/keys/priv.pem", fs.constants.R_OK);
            await fs.access("./server/keys/pub.pem", fs.constants.R_OK);
            return true;
        } catch {
            return false;
        }
    ...
    async importKeys() {
        const priv = await fs.readFile("./server/keys/priv.pem", { encoding: "utf8" });
        const pub = await fs.readFile("./server/keys/pub.pem", { encoding: "utf8" });
    ...
    async generateAndSaveKeys() {
        ...
        await fs.writeFile("./server/keys/priv.pem", priv, { encoding: "utf8" });
        await fs.writeFile("./server/keys/pub.pem", pub, { encoding: "utf8" });
    }
